...
page_title: "elestio_hydra Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  Hydra is an open-source alternative to enterprise data warehouses and it's simple, fast, and adaptable to your needs.The elestio_hydra resource allows the creation and management of Elestio Hydra services. The service uses the following docker image hydradatabase/hydra https://hub.docker.com/r/hydradatabase/hydra
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `443/tcp`, `15432/tcp`, `80/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a Hydra service by specifying the Project ID it belongs to, and the Hydra service ID (spaced by a comma).
terraform import elestio_hydra.my_hydra project_id,hydra_service_id
```
