...
description: |-
  The Industry Standard In IT Infrastructure MonitoringThe elestio_nagios resource allows the creation and management of Elestio Nagios services. The service uses the following docker image jasonrivers/nagios https://hub.docker.com/r/jasonrivers/nagios
---
...
```shell
# Import a Nagios service by specifying the Project ID it belongs to, and the Nagios service ID (spaced by a comma).
terraform import elestio_nagios.my_nagios project_id,nagios_service_id
```
