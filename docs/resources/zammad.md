...
description: |-
  Zammad is a web-based, open-source helpdesk/customer support system.The elestio_zammad resource allows the creation and management of Elestio Zammad services. The service uses the following docker image zammad/zammad-docker-compose https://hub.docker.com/r/zammad/zammad-docker-compose
---
...
- `version` (String) This is the version of the software used as service. **Default** `5.4.1-38`.
...
```shell
# Import a Zammad service by specifying the Project ID it belongs to, and the Zammad service ID (spaced by a comma).
terraform import elestio_zammad.my_zammad project_id,zammad_service_id
```
