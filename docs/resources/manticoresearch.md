...
description: |-
  Manticore Search is an easy to use open source fast database for search. Modern, fast, light-weight, outstanding full-text search capabilities.The elestio_manticoresearch resource allows the creation and management of Elestio ManticoreSearch services. The service uses the following docker image manticoresearch/manticore https://hub.docker.com/r/manticoresearch/manticore
---
...
```shell
# Import a ManticoreSearch service by specifying the Project ID it belongs to, and the ManticoreSearch service ID (spaced by a comma).
terraform import elestio_manticoresearch.my_manticoresearch project_id,manticoresearch_service_id
```
