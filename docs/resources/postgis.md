...
page_title: "elestio_postgis Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  PostGIS extends the capabilities of the PostgreSQL relational database by adding support for storing, indexing, and querying geospatial data.The elestio_postgis resource allows the creation and management of Elestio postGis services. The service uses the following docker image postgis/postgis https://hub.docker.com/r/postgis/postgis
---

# elestio_postgis (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/postGis-Vy8g.png" width="100" /><br>PostGIS extends the capabilities of the PostgreSQL relational database by adding support for storing, indexing, and querying geospatial data.<br><br>The **elestio_postgis** resource allows the creation and management of Elestio postGis services. The service uses the following docker image [postgis/postgis](https://hub.docker.com/r/postgis/postgis)
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `35432/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `17-master`.
...
```shell
# Import a postGis service by specifying the Project ID it belongs to, and the postGis service ID (spaced by a comma).
terraform import elestio_postgis.my_postgis project_id,postgis_service_id
```
