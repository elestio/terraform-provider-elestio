...
- `version` (String) This is the version of the software used as service. **Default** `v1.0.3`.
...
```shell
# Import a Firefish service by specifying the Project ID it belongs to, and the Firefish service ID (spaced by a comma).
terraform import elestio_firefish.my_firefish project_id,firefish_service_id
```
