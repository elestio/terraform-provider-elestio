...
description: |-
  Trilium Notes is a hierarchical note taking application with focus on building large personal knowledge bases.The elestio_trilium resource allows the creation and management of Elestio Trilium services. The service uses the following docker image zadam/trilium https://hub.docker.com/r/zadam/trilium
---
...
```shell
# Import a Trilium service by specifying the Project ID it belongs to, and the Trilium service ID (spaced by a comma).
terraform import elestio_trilium.my_trilium project_id,trilium_service_id
```
