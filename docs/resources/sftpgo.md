...
description: |-
  Fully featured and highly configurable SFTP server with optional HTTP/S, FTP/S and WebDAV supportThe elestio_sftpgo resource allows the creation and management of Elestio SFTPGo services. The service uses the following docker image drakkan/sftpgo https://hub.docker.com/r/drakkan/sftpgo
---
...
```shell
# Import a SFTPGo service by specifying the Project ID it belongs to, and the SFTPGo service ID (spaced by a comma).
terraform import elestio_sftpgo.my_sftpgo project_id,sftpgo_service_id
```
