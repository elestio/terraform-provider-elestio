...
description: |-
  immutable ledger database based on zero trust, SQL and Key-Value, tamperproof, data change historyThe elestio_immudb resource allows the creation and management of Elestio ImmuDB services. The service uses the following docker image codenotary/immudb https://hub.docker.com/r/codenotary/immudb
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `3322/tcp`, `9497/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a ImmuDB service by specifying the Project ID it belongs to, and the ImmuDB service ID (spaced by a comma).
terraform import elestio_immudb.my_immudb project_id,immudb_service_id
```
