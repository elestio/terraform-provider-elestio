...
description: |-
  The Reliable, High Performance TCP/HTTP Load BalancerThe elestio_haproxy resource allows the creation and management of Elestio HAProxy services. The service uses the following docker image haproxytech/haproxy-debian https://hub.docker.com/r/haproxytech/haproxy-debian
---
...
```shell
# Import a HAProxy service by specifying the Project ID it belongs to, and the HAProxy service ID (spaced by a comma).
terraform import elestio_haproxy.my_haproxy project_id,haproxy_service_id
```
