...
description: |-
  Ackee is a self-hosted, Node.js-based analytics tool for ensuring privacy.The elestio_ackee resource allows the creation and management of Elestio Ackee services. The service uses the following docker image electerious/ackee https://hub.docker.com/r/electerious/ackee
---
...
```shell
# Import a Ackee service by specifying the Project ID it belongs to, and the Ackee service ID (spaced by a comma).
terraform import elestio_ackee.my_ackee project_id,ackee_service_id
```
