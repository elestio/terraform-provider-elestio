...
description: |-
  Changedetection.io is an open source web page monitoring, notification and change detection.The elestio_changedetection resource allows the creation and management of Elestio ChangeDetection services. The service uses the following docker image dgtlmoon/changedetection.io https://hub.docker.com/r/dgtlmoon/changedetection.io
---
...
```shell
# Import a ChangeDetection service by specifying the Project ID it belongs to, and the ChangeDetection service ID (spaced by a comma).
terraform import elestio_changedetection.my_changedetection project_id,changedetection_service_id
```
