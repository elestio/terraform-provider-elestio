...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `27017/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a FerretDB service by specifying the Project ID it belongs to, and the FerretDB service ID (spaced by a comma).
terraform import elestio_ferretdb.my_ferretdb project_id,ferretdb_service_id
```
