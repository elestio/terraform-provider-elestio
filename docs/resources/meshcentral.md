...
description: |-
  MeshCentral is a full computer management web siteThe elestio_meshcentral resource allows the creation and management of Elestio MeshCentral services. The service uses the following docker image polonel/trudesk https://hub.docker.com/r/polonel/trudesk
---
...
```shell
# Import a MeshCentral service by specifying the Project ID it belongs to, and the MeshCentral service ID (spaced by a comma).
terraform import elestio_meshcentral.my_meshcentral project_id,meshcentral_service_id
```
