...
description: |-
  FreeScout is a free self-hosted help desk & shared mailbox.The elestio_freescout resource allows the creation and management of Elestio FreeScout services. The service uses the following docker image tiredofit/freescout https://hub.docker.com/r/tiredofit/freescout
---
...
```shell
# Import a FreeScout service by specifying the Project ID it belongs to, and the FreeScout service ID (spaced by a comma).
terraform import elestio_freescout.my_freescout project_id,freescout_service_id
```
