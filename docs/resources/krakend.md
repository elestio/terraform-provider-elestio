...
page_title: "elestio_krakend Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  KrakenD is a stateless, distributed, high-performance API Gateway that helps you effortlessly adopt microservices · The API Gateway pattern at its full extent.The elestio_krakend resource allows the creation and management of Elestio KrakenD services. The service uses the following docker image devopsfaith/krakend https://hub.docker.com/r/devopsfaith/krakend
---
...
```shell
# Import a KrakenD service by specifying the Project ID it belongs to, and the KrakenD service ID (spaced by a comma).
terraform import elestio_krakend.my_krakend project_id,krakend_service_id
```
