...
description: |-
  A self-hosted web radio management suiteThe elestio_azuracast resource allows the creation and management of Elestio AzuraCast services. The service uses the following docker image azuracast/azuracast https://hub.docker.com/r/azuracast/azuracast
---

# elestio_azuracast (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/AzuraCast-aDBt.png" width="100" /><br>A self-hosted web radio management suite<br><br>The **elestio_azuracast** resource allows the creation and management of Elestio AzuraCast services. The service uses the following docker image [azuracast/azuracast](https://hub.docker.com/r/azuracast/azuracast)
...
```shell
# Import a AzuraCast service by specifying the Project ID it belongs to, and the AzuraCast service ID (spaced by a comma).
terraform import elestio_azuracast.my_azuracast project_id,azuracast_service_id
```
