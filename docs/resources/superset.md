...
description: |-
  Apache Superset is a data visualization and data exploration platform.The elestio_superset resource allows the creation and management of Elestio Superset services. The service uses the following docker image apache/superset https://hub.docker.com/r/apache/superset
---
...
- `version` (String) This is the version of the software used as service. **Default** `master`.
...
```shell
# Import a Superset service by specifying the Project ID it belongs to, and the Superset service ID (spaced by a comma).
terraform import elestio_superset.my_superset project_id,superset_service_id
```
