...
page_title: "elestio_documenso Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  The Open Source DocuSign Alternative, Document signing.The elestio_documenso resource allows the creation and management of Elestio Documenso services. The service uses the following docker image documenso/documenso https://hub.docker.com/r/documenso/documenso
---

# elestio_documenso (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/Documenso-zSQc.png" width="100" /><br>The Open Source DocuSign Alternative, Document signing.<br><br>The **elestio_documenso** resource allows the creation and management of Elestio Documenso services. The service uses the following docker image [documenso/documenso](https://hub.docker.com/r/documenso/documenso)
...
```shell
# Import a Documenso service by specifying the Project ID it belongs to, and the Documenso service ID (spaced by a comma).
terraform import elestio_documenso.my_documenso project_id,documenso_service_id
```
