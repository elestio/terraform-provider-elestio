...
description: |-
  QuickChart allows you to generates chart images of charts from a URL. The elestio_quickchart resource allows the creation and management of Elestio QuickChart services. The service uses the following docker image ianw/quickchart https://hub.docker.com/r/ianw/quickchart
---
...
```shell
# Import a QuickChart service by specifying the Project ID it belongs to, and the QuickChart service ID (spaced by a comma).
terraform import elestio_quickchart.my_quickchart project_id,quickchart_service_id
```
