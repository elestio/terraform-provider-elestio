...
page_title: "elestio_pgduckdb Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  DuckDB's columnar-vectorized analytics engine into PostgreSQL, enabling high-performance analytics and data-intensive applicationsThe elestio_pgduckdb resource allows the creation and management of Elestio pgDuckDB services. The service uses the following docker image pgduckdb/pgduckdb https://hub.docker.com/r/pgduckdb/pgduckdb
---

# elestio_pgduckdb (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/pgDuckDB-hzgt.png" width="100" /><br>DuckDB's columnar-vectorized analytics engine into PostgreSQL, enabling high-performance analytics and data-intensive applications<br><br>The **elestio_pgduckdb** resource allows the creation and management of Elestio pgDuckDB services. The service uses the following docker image [pgduckdb/pgduckdb](https://hub.docker.com/r/pgduckdb/pgduckdb)
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `25432/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `18-main`.
...
```shell
# Import a pgDuckDB service by specifying the Project ID it belongs to, and the pgDuckDB service ID (spaced by a comma).
terraform import elestio_pgduckdb.my_pgduckdb project_id,pgduckdb_service_id
```
