...
description: |-
  ArchiveBox is a powerful, self-hosted internet archiving solution to collect, save, and view sites you want to preserve offline.The elestio_archivebox resource allows the creation and management of Elestio ArchiveBox services. The service uses the following docker image archivebox/archivebox https://hub.docker.com/r/archivebox/archivebox
---
...
```shell
# Import a ArchiveBox service by specifying the Project ID it belongs to, and the ArchiveBox service ID (spaced by a comma).
terraform import elestio_archivebox.my_archivebox project_id,archivebox_service_id
```
