...
description: |-
  A fully functional local cloud stack. Develop and test your cloud and serverless apps offline!The elestio_localstack resource allows the creation and management of Elestio LocalStack services. The service uses the following docker image localstack/localstack https://hub.docker.com/r/localstack/localstack
---
...
```shell
# Import a LocalStack service by specifying the Project ID it belongs to, and the LocalStack service ID (spaced by a comma).
terraform import elestio_localstack.my_localstack project_id,localstack_service_id
```
