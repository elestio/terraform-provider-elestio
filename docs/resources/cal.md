...
- `version` (String) This is the version of the software used as service. **Default** `v5.6.18`.
...
```shell
# Import a Cal service by specifying the Project ID it belongs to, and the Cal service ID (spaced by a comma).
terraform import elestio_cal.my_cal project_id,cal_service_id
```
