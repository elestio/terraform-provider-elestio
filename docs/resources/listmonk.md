...
description: |-
  Listmonk is a self-hosted, high performance mailing list and newsletter managerThe elestio_listmonk resource allows the creation and management of Elestio Listmonk services. The service uses the following docker image listmonk/listmonk https://hub.docker.com/r/listmonk/listmonk
---
...
```shell
# Import a Listmonk service by specifying the Project ID it belongs to, and the Listmonk service ID (spaced by a comma).
terraform import elestio_listmonk.my_listmonk project_id,listmonk_service_id
```
