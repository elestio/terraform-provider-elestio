...
page_title: "elestio_apitable Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  APITable is an API-oriented low-code platform for building collaborative apps and is better than all other Airtable open-source alternatives.The elestio_apitable resource allows the creation and management of Elestio APITable services. The service uses the following docker image apitable/web-servers https://hub.docker.com/r/apitable/web-servers
---
...
```shell
# Import a APITable service by specifying the Project ID it belongs to, and the APITable service ID (spaced by a comma).
terraform import elestio_apitable.my_apitable project_id,apitable_service_id
```
