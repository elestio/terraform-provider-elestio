...
page_title: "elestio_columnstore Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  MariaDB ColumnStore is a GPLv2 open-source columnar database built on MariaDB Server.The elestio_columnstore resource allows the creation and management of Elestio ColumnStore services. The service uses the following docker image mariadb/columnstore https://hub.docker.com/r/mariadb/columnstore
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `23060/tcp`, `443/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a ColumnStore service by specifying the Project ID it belongs to, and the ColumnStore service ID (spaced by a comma).
terraform import elestio_columnstore.my_columnstore project_id,columnstore_service_id
```
