...
subcategory: ""
description: |-
  Nomad is a scheduler and workload orchestrator.elestio_nomad is a preconfigured elestio_service resource (template_id: 170) running Nomad.
---
...
```shell
# Import a Nomad service by specifying the Project ID it belongs to, and the Nomad service ID (spaced by a comma).
terraform import elestio_nomad.my_nomad project_id,nomad_service_id
```
