...
description: |-
  Open source virtual / remote desktop infrastructure for everyone!The elestio_rustdeskserver resource allows the creation and management of Elestio RustdeskServer services. The service uses the following docker image rustdesk/rustdesk-server https://hub.docker.com/r/rustdesk/rustdesk-server
---
...
```shell
# Import a RustdeskServer service by specifying the Project ID it belongs to, and the RustdeskServer service ID (spaced by a comma).
terraform import elestio_rustdeskserver.my_rustdeskserver project_id,rustdeskserver_service_id
```
