...
```shell
# Import a Presidio service by specifying the Project ID it belongs to, and the Presidio service ID (spaced by a comma).
terraform import elestio_presidio.my_presidio project_id,presidio_service_id
```
