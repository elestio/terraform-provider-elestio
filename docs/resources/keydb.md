...
description: |-
  KeyDB is both your cache and database, for cloud-optimized solutions.The elestio_keydb resource allows the creation and management of Elestio KeyDB services. The service uses the following docker image eqalpha/keydb https://hub.docker.com/r/eqalpha/keydb
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `23647/tcp`, `24814/tcp`, `26380/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a KeyDB service by specifying the Project ID it belongs to, and the KeyDB service ID (spaced by a comma).
terraform import elestio_keydb.my_keydb project_id,keydb_service_id
```
