...
description: |-
  OpenLDAP is a free, open-source implementation of the Lightweight Directory Access Protocol (LDAP)The elestio_openldap resource allows the creation and management of Elestio OpenLDAP services. The service uses the following docker image osixia/openldap https://hub.docker.com/r/osixia/openldap
---
...
```shell
# Import a OpenLDAP service by specifying the Project ID it belongs to, and the OpenLDAP service ID (spaced by a comma).
terraform import elestio_openldap.my_openldap project_id,openldap_service_id
```
