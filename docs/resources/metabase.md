...
description: |-
  Metabase is an open-source tool that simply and quickly gathers business intelligence and analytics for your company.The elestio_metabase resource allows the creation and management of Elestio Metabase services. The service uses the following docker image metabase/metabase https://hub.docker.com/r/metabase/metabase
---
...
```shell
# Import a Metabase service by specifying the Project ID it belongs to, and the Metabase service ID (spaced by a comma).
terraform import elestio_metabase.my_metabase project_id,metabase_service_id
```
