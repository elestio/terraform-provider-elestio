...
description: |-
  Cronicle is a simple, distributed task scheduler and runner with a web based UI.The elestio_cronicle resource allows the creation and management of Elestio Cronicle services. The service uses the following docker image btdwv/cronicle https://hub.docker.com/r/btdwv/cronicle
---
...
```shell
# Import a Cronicle service by specifying the Project ID it belongs to, and the Cronicle service ID (spaced by a comma).
terraform import elestio_cronicle.my_cronicle project_id,cronicle_service_id
```
