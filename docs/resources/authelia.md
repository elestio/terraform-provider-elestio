...
page_title: "elestio_authelia Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  Authelia is an authentication and authorization server and portal fulfilling the identity and access management (IAM) role of information security in providing multi-factor authentication and single sign-on (SSO) for your applications via a web portal. ItThe elestio_authelia resource allows the creation and management of Elestio Authelia services. The service uses the following docker image authelia/authelia https://hub.docker.com/r/authelia/authelia
---
...
```shell
# Import a Authelia service by specifying the Project ID it belongs to, and the Authelia service ID (spaced by a comma).
terraform import elestio_authelia.my_authelia project_id,authelia_service_id
```
