...
page_title: "elestio_pgvector Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  Open-source vector similarity search for PostgresThe elestio_pgvector resource allows the creation and management of Elestio pgvector services. The service uses the following docker image pgvector/pgvector https://hub.docker.com/r/pgvector/pgvector
---

# elestio_pgvector (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/pgvector-1TOp.png" width="100" /><br>Open-source vector similarity search for Postgres<br><br>The **elestio_pgvector** resource allows the creation and management of Elestio pgvector services. The service uses the following docker image [pgvector/pgvector](https://hub.docker.com/r/pgvector/pgvector)
...
- `version` (String) This is the version of the software used as service. **Default** `pg17`.
...
```shell
# Import a pgvector service by specifying the Project ID it belongs to, and the pgvector service ID (spaced by a comma).
terraform import elestio_pgvector.my_pgvector project_id,pgvector_service_id
```
