...
description: |-
  MantisBT makes collaboration with team members & clients easy, fast, and professionalThe elestio_mantisbt resource allows the creation and management of Elestio MantisBT services. The service uses the following docker image xlrl/mantisbt https://hub.docker.com/r/xlrl/mantisbt
---
...
```shell
# Import a MantisBT service by specifying the Project ID it belongs to, and the MantisBT service ID (spaced by a comma).
terraform import elestio_mantisbt.my_mantisbt project_id,mantisbt_service_id
```
