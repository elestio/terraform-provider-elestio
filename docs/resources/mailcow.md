...
description: |-
  Mailcow is a mail server suite with an elegant web interface for managing domains, mailboxes, etc.The elestio_mailcow resource allows the creation and management of Elestio MailCow services. The service uses the following docker image mailcow https://hub.docker.com/r/mailcow
---
...
```shell
# Import a MailCow service by specifying the Project ID it belongs to, and the MailCow service ID (spaced by a comma).
terraform import elestio_mailcow.my_mailcow project_id,mailcow_service_id
```
