...
page_title: "elestio_clickhouses3 Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  ClickHouse + S3  is an open-source, column-oriented DBMS for online analytical processing. The elestio_clickhouses3 resource allows the creation and management of Elestio ClickHouseS3 services. The service uses the following docker image clickhouse/clickhouse-server https://hub.docker.com/r/clickhouse/clickhouse-server
---

# elestio_clickhouses3 (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/clickhouse-WFqc.png" width="100" /><br>ClickHouse + S3  is an open-source, column-oriented DBMS for online analytical processing. <br><br>The **elestio_clickhouses3** resource allows the creation and management of Elestio ClickHouseS3 services. The service uses the following docker image [clickhouse/clickhouse-server](https://hub.docker.com/r/clickhouse/clickhouse-server)
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `18123/tcp`, `29000/tcp`, `24306/tcp`, `25432/tcp`, `34256/tcp`, `20001/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a ClickHouseS3 service by specifying the Project ID it belongs to, and the ClickHouseS3 service ID (spaced by a comma).
terraform import elestio_clickhouses3.my_clickhouses3 project_id,clickhouses3_service_id
```
