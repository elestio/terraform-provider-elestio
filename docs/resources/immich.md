...
- `version` (String) This is the version of the software used as service. **Default** `release`.
...
```shell
# Import a Immich service by specifying the Project ID it belongs to, and the Immich service ID (spaced by a comma).
terraform import elestio_immich.my_immich project_id,immich_service_id
```
