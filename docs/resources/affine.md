...
subcategory: ""
description: |-
  More than Notion and Miro's next-generation knowledge base, AFFiNE unifies planning, sorting, and creation.elestio_affine is a preconfigured elestio_service resource (template_id: 294) running Affine from the Docker image https://hub.docker.com/r/ghcr.io/toeverything/affine-self-hosted ghcr.io/toeverything/affine-self-hosted.
---
...
```shell
# Import a Affine service by specifying the Project ID it belongs to, and the Affine service ID (spaced by a comma).
terraform import elestio_affine.my_affine project_id,affine_service_id
```
