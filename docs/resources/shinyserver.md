...
page_title: "elestio_shinyserver Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  Shiny Server (formerly RStudio) for hosting R Shiny web applications.The elestio_shinyserver resource allows the creation and management of Elestio ShinyServer services. The service uses the following docker image rocker/shiny https://hub.docker.com/r/rocker/shiny
---

# elestio_shinyserver (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/ShinyServer-o1KW.png" width="100" /><br>Shiny Server (formerly RStudio) for hosting R Shiny web applications.<br><br>The **elestio_shinyserver** resource allows the creation and management of Elestio ShinyServer services. The service uses the following docker image [rocker/shiny](https://hub.docker.com/r/rocker/shiny)
...
```shell
# Import a ShinyServer service by specifying the Project ID it belongs to, and the ShinyServer service ID (spaced by a comma).
terraform import elestio_shinyserver.my_shinyserver project_id,shinyserver_service_id
```
