...
description: |-
  K3s is a fully compliant Kubernetes distributionThe elestio_k3s resource allows the creation and management of Elestio K3S services.
---
...
```shell
# Import a K3S service by specifying the Project ID it belongs to, and the K3S service ID (spaced by a comma).
terraform import elestio_k3s.my_k3s project_id,k3s_service_id
```
