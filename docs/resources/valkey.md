...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `26379/tcp`, `80/tcp`, `443/tcp`, `26380/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a Valkey service by specifying the Project ID it belongs to, and the Valkey service ID (spaced by a comma).
terraform import elestio_valkey.my_valkey project_id,valkey_service_id
```
