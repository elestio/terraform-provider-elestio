...
- `version` (String) This is the version of the software used as service. **Default** `v1.6.0`.
...
```shell
# Import a Gradio service by specifying the Project ID it belongs to, and the Gradio service ID (spaced by a comma).
terraform import elestio_gradio.my_gradio project_id,gradio_service_id
```
