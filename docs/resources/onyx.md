...
page_title: "elestio_onyx Resource - terraform-provider-elestio"
subcategory: "Services: AI/GPU"
description: |-
  AI Platform - AI Chat with advanced features that works with every LLMThe elestio_onyx resource allows the creation and management of Elestio Onyx services. The service uses the following docker image onyxdotapp/onyx-web-server https://hub.docker.com/r/onyxdotapp/onyx-web-server
---

# elestio_onyx (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/onyx-oeVO.png" width="100" /><br>AI Platform - AI Chat with advanced features that works with every LLM<br><br>The **elestio_onyx** resource allows the creation and management of Elestio Onyx services. The service uses the following docker image [onyxdotapp/onyx-web-server](https://hub.docker.com/r/onyxdotapp/onyx-web-server)
...
```shell
# Import a Onyx service by specifying the Project ID it belongs to, and the Onyx service ID (spaced by a comma).
terraform import elestio_onyx.my_onyx project_id,onyx_service_id
```
