...
description: |-
  Budibase is a quick way to build business apps that empower teams and improve productivity. The elestio_budibase resource allows the creation and management of Elestio Budibase services. The service uses the following docker image budibase/apps https://hub.docker.com/r/budibase/apps
---
...
- `version` (String) This is the version of the software used as service. **Default** `3.7.2`.
...
```shell
# Import a Budibase service by specifying the Project ID it belongs to, and the Budibase service ID (spaced by a comma).
terraform import elestio_budibase.my_budibase project_id,budibase_service_id
```
