...
description: |-
  MediaCMS is a fully featured open-source video and media CMS.The elestio_mediacms resource allows the creation and management of Elestio MediaCMS services. The service uses the following docker image mediacms/mediacms https://hub.docker.com/r/mediacms/mediacms
---
...
```shell
# Import a MediaCMS service by specifying the Project ID it belongs to, and the MediaCMS service ID (spaced by a comma).
terraform import elestio_mediacms.my_mediacms project_id,mediacms_service_id
```
