...
```shell
# Import a FileRun service by specifying the Project ID it belongs to, and the FileRun service ID (spaced by a comma).
terraform import elestio_filerun.my_filerun project_id,filerun_service_id
```
