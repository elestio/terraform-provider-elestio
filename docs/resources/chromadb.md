...
page_title: "elestio_chromadb Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  The AI-native open-source embedding databaseThe elestio_chromadb resource allows the creation and management of Elestio Chromadb services. The service uses the following docker image chromadb/chroma https://hub.docker.com/r/chromadb/chroma
---

# elestio_chromadb (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/chromadb-3v4X.png" width="100" /><br>The AI-native open-source embedding database<br><br>The **elestio_chromadb** resource allows the creation and management of Elestio Chromadb services. The service uses the following docker image [chromadb/chroma](https://hub.docker.com/r/chromadb/chroma)
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `44751/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `0.6.1`.
...
```shell
# Import a Chromadb service by specifying the Project ID it belongs to, and the Chromadb service ID (spaced by a comma).
terraform import elestio_chromadb.my_chromadb project_id,chromadb_service_id
```
