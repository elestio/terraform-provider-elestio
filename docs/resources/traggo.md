...
page_title: "elestio_traggo Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  Traggo is a tag-based time tracking tool. In Traggo there are no tasks, only tagged time spans.The elestio_traggo resource allows the creation and management of Elestio Traggo services. The service uses the following docker image traggo/server https://hub.docker.com/r/traggo/server
---
...
```shell
# Import a Traggo service by specifying the Project ID it belongs to, and the Traggo service ID (spaced by a comma).
terraform import elestio_traggo.my_traggo project_id,traggo_service_id
```
