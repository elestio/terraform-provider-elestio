...
description: |-
  InfluxDB is a scalable datastore that empowers developers to build IoT, analytics and monitoring software. The elestio_influxdb resource allows the creation and management of Elestio InfluxDB services. The service uses the following docker image bitnami/influxdb https://hub.docker.com/r/bitnami/influxdb
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
