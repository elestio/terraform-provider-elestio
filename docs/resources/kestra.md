...
description: |-
  The open source data orchestration and scheduling platform.The elestio_kestra resource allows the creation and management of Elestio Kestra services. The service uses the following docker image kestra/kestra https://hub.docker.com/r/kestra/kestra
---
...
```shell
# Import a Kestra service by specifying the Project ID it belongs to, and the Kestra service ID (spaced by a comma).
terraform import elestio_kestra.my_kestra project_id,kestra_service_id
```
