...
description: |-
  Wikijs is a powerful open-source wiki app built on Node.js, Git and Markdown.The elestio_wikijs resource allows the creation and management of Elestio Wikijs services. The service uses the following docker image requarks/wiki https://hub.docker.com/r/requarks/wiki
---
...
```shell
# Import a Wikijs service by specifying the Project ID it belongs to, and the Wikijs service ID (spaced by a comma).
terraform import elestio_wikijs.my_wikijs project_id,wikijs_service_id
```
