...
```shell
# Import a Parse service by specifying the Project ID it belongs to, and the Parse service ID (spaced by a comma).
terraform import elestio_parse.my_parse project_id,parse_service_id
```
