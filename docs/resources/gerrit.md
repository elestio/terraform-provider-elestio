...
description: |-
  Gerrit is a code review and project management tool for Git based projects.The elestio_gerrit resource allows the creation and management of Elestio Gerrit services. The service uses the following docker image gerritcodereview/gerrit https://hub.docker.com/r/gerritcodereview/gerrit
---
...
```shell
# Import a Gerrit service by specifying the Project ID it belongs to, and the Gerrit service ID (spaced by a comma).
terraform import elestio_gerrit.my_gerrit project_id,gerrit_service_id
```
