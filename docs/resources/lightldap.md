...
page_title: "elestio_lightldap Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  A Lightweight authentication server that provides an opinionated, simplified LDAP interface for authentication. It integrates with many backends, from KeyCloak to Authelia to Nextcloud and more!The elestio_lightldap resource allows the creation and management of Elestio LightLDAP services. The service uses the following docker image nitnelave/lldap https://hub.docker.com/r/nitnelave/lldap
---
...
```shell
# Import a LightLDAP service by specifying the Project ID it belongs to, and the LightLDAP service ID (spaced by a comma).
terraform import elestio_lightldap.my_lightldap project_id,lightldap_service_id
```
