...
description: |-
  Syncthing is a free, continuous, multiple-device file synchronization program. The elestio_syncthing resource allows the creation and management of Elestio Syncthing services. The service uses the following docker image syncthing/syncthing https://hub.docker.com/r/syncthing/syncthing
---
...
```shell
# Import a Syncthing service by specifying the Project ID it belongs to, and the Syncthing service ID (spaced by a comma).
terraform import elestio_syncthing.my_syncthing project_id,syncthing_service_id
```
