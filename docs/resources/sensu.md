...
description: |-
  Sensu is an open source monitoring tool for ephemeral infrastructure and distributed applications.The elestio_sensu resource allows the creation and management of Elestio Sensu services. The service uses the following docker image sensu/sensu https://hub.docker.com/r/sensu/sensu
---
...
```shell
# Import a Sensu service by specifying the Project ID it belongs to, and the Sensu service ID (spaced by a comma).
terraform import elestio_sensu.my_sensu project_id,sensu_service_id
```
