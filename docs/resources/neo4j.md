...
description: |-
  Neo4j is the world’s leading Graph DatabaseThe elestio_neo4j resource allows the creation and management of Elestio Neo4j services. The service uses the following docker image neo4j https://hub.docker.com/r/neo4j
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `7687/tcp`, `27687/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a Neo4j service by specifying the Project ID it belongs to, and the Neo4j service ID (spaced by a comma).
terraform import elestio_neo4j.my_neo4j project_id,neo4j_service_id
```
