...
description: |-
  PowerDNS is a premier supplier of open-source DNS software, services and support. The elestio_powerdns resource allows the creation and management of Elestio PowerDNS services. The service uses the following docker image pschiffe/pdns-mysql https://hub.docker.com/r/pschiffe/pdns-mysql
---
...
```shell
# Import a PowerDNS service by specifying the Project ID it belongs to, and the PowerDNS service ID (spaced by a comma).
terraform import elestio_powerdns.my_powerdns project_id,powerdns_service_id
```
