...
- `version` (String) This is the version of the software used as service. **Default** `v7.81`.
...
```shell
# Import a Wekan service by specifying the Project ID it belongs to, and the Wekan service ID (spaced by a comma).
terraform import elestio_wekan.my_wekan project_id,wekan_service_id
```
