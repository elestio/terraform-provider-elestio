...
description: |-
  ScyllaDB is a true NoSQL database for the most demanding applications. The elestio_scylladb resource allows the creation and management of Elestio ScyllaDB services. The service uses the following docker image scylladb/scylla https://hub.docker.com/r/scylladb/scylla
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `26443/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a ScyllaDB service by specifying the Project ID it belongs to, and the ScyllaDB service ID (spaced by a comma).
terraform import elestio_scylladb.my_scylladb project_id,scylladb_service_id
```
