...
- `version` (String) This is the version of the software used as service. **Default** `20250210-1`.
...
```shell
# Import a Revolt service by specifying the Project ID it belongs to, and the Revolt service ID (spaced by a comma).
terraform import elestio_revolt.my_revolt project_id,revolt_service_id
```
