...
page_title: "elestio_loki Resource - terraform-provider-elestio"
subcategory: "Services: Hosting & Infra"
description: |-
  Loki is a log aggregation system designed to store and query logs from all your applications and infrastructure.The elestio_loki resource allows the creation and management of Elestio Loki services. The service uses the following docker image grafana/loki https://hub.docker.com/r/grafana/loki
---
...
- `version` (String) This is the version of the software used as service. **Default** `2.8.0`.
...
```shell
# Import a Loki service by specifying the Project ID it belongs to, and the Loki service ID (spaced by a comma).
terraform import elestio_loki.my_loki project_id,loki_service_id
```
