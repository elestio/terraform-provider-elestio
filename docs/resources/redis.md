...
```shell
# Import a Redis service by specifying the Project ID it belongs to, and the Redis service ID (spaced by a comma).
terraform import elestio_redis.my_redis project_id,redis_service_id
```
