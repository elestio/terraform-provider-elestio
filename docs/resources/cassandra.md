...
description: |-
  Open Source NoSQL DatabaseThe elestio_cassandra resource allows the creation and management of Elestio Cassandra services. The service uses the following docker image bitnami/cassandra https://hub.docker.com/r/bitnami/cassandra
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `7000/tcp`, `9042/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a Cassandra service by specifying the Project ID it belongs to, and the Cassandra service ID (spaced by a comma).
terraform import elestio_cassandra.my_cassandra project_id,cassandra_service_id
```
