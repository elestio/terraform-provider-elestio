...
description: |-
  BookStack is a platform to create documentation/wiki content built with PHP & Laravel.The elestio_bookstack resource allows the creation and management of Elestio BookStack services. The service uses the following docker image linuxserver/bookstack https://hub.docker.com/r/linuxserver/bookstack
---
...
```shell
# Import a BookStack service by specifying the Project ID it belongs to, and the BookStack service ID (spaced by a comma).
terraform import elestio_bookstack.my_bookstack project_id,bookstack_service_id
```
