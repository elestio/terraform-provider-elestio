...
description: |-
  Open source distributed and RESTful search engine.The elestio_opensearch resource allows the creation and management of Elestio OpenSearch services. The service uses the following docker image opensearchproject/opensearch https://hub.docker.com/r/opensearchproject/opensearch
---
...
```shell
# Import a OpenSearch service by specifying the Project ID it belongs to, and the OpenSearch service ID (spaced by a comma).
terraform import elestio_opensearch.my_opensearch project_id,opensearch_service_id
```
