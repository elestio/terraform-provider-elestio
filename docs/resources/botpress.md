...
page_title: "elestio_botpress Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  The first next-generation chatbot builder powered by OpenAI. Build ChatGPT-like bots for your project or business to get things done. The elestio_botpress resource allows the creation and management of Elestio Botpress services. The service uses the following docker image botpress/server https://hub.docker.com/r/botpress/server
---
...
```shell
# Import a Botpress service by specifying the Project ID it belongs to, and the Botpress service ID (spaced by a comma).
terraform import elestio_botpress.my_botpress project_id,botpress_service_id
```
