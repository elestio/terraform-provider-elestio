...
description: |-
  Start Accepting Bitcoin Payments With 0% Fees & No Third-partyThe elestio_btcpay resource allows the creation and management of Elestio BTCPay services. The service uses the following docker image btcpayserver/btcpayserver https://hub.docker.com/r/btcpayserver/btcpayserver
---
...
```shell
# Import a BTCPay service by specifying the Project ID it belongs to, and the BTCPay service ID (spaced by a comma).
terraform import elestio_btcpay.my_btcpay project_id,btcpay_service_id
```
