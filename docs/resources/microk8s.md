...
description: |-
  MicroK8s is a small, fast, single-package Kubernetes for developers, IoT and edgeThe elestio_microk8s resource allows the creation and management of Elestio Microk8s services.
---
...
```shell
# Import a Microk8s service by specifying the Project ID it belongs to, and the Microk8s service ID (spaced by a comma).
terraform import elestio_microk8s.my_microk8s project_id,microk8s_service_id
```
