...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `55701/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `3-core`.
...
```shell
# Import a InfluxDB service by specifying the Project ID it belongs to, and the InfluxDB service ID (spaced by a comma).
terraform import elestio_influxdb.my_influxdb project_id,influxdb_service_id
```
