...
- `version` (String) This is the version of the software used as service. **Default** `1.32.3746.95`.
...
```shell
# Import a Pritunl service by specifying the Project ID it belongs to, and the Pritunl service ID (spaced by a comma).
terraform import elestio_pritunl.my_pritunl project_id,pritunl_service_id
```
