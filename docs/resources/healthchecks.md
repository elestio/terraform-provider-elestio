...
<img src="//cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/logo-healthchecks-zDNH.svg" width="100" height="100" /><br/> Healthchecks is an online service for monitoring regularly running tasks such as cron jobs.<br/><br/>**elestio_healthchecks** is a preconfigured elestio_service resource (`template_id: 140`) running **Healthchecks** ([`docker_image: linuxserver/healthchecks`](https://hub.docker.com/r/linuxserver/healthchecks)).
...
```shell
# Import a Healthchecks service by specifying the Project ID it belongs to, and the Healthchecks service ID (spaced by a comma).
terraform import elestio_healthchecks.my_healthchecks project_id,healthchecks_service_id
```
