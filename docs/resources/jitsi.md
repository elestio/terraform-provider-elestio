...
description: |-
  Jitsi is a set of open-source projects that allows you to easily build and deploy secure video conferencing solutions. The elestio_jitsi resource allows the creation and management of Elestio Jitsi services. The service uses the following docker image jitsi/web https://hub.docker.com/r/jitsi/web
---
...
```shell
# Import a Jitsi service by specifying the Project ID it belongs to, and the Jitsi service ID (spaced by a comma).
terraform import elestio_jitsi.my_jitsi project_id,jitsi_service_id
```
