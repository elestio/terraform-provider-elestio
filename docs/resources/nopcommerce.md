...
description: |-
  nopCommerce is the most popular ASP.NET shopping cart in the world based on Microsoft technologies. Free and open-source eCommerce platform.The elestio_nopcommerce resource allows the creation and management of Elestio nopCommerce services. The service uses the following docker image elestio/nopcommerce https://hub.docker.com/r/elestio/nopcommerce
---
...
```shell
# Import a nopCommerce service by specifying the Project ID it belongs to, and the nopCommerce service ID (spaced by a comma).
terraform import elestio_nopcommerce.my_nopcommerce project_id,nopcommerce_service_id
```
