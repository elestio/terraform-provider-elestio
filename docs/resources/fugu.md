...
description: |-
  Fugu is a product analytics software with a strong focus on simplicity and privacy-friendliness.The elestio_fugu resource allows the creation and management of Elestio Fugu services. The service uses the following docker image shafyy/fugu https://hub.docker.com/r/shafyy/fugu
---
...
```shell
# Import a Fugu service by specifying the Project ID it belongs to, and the Fugu service ID (spaced by a comma).
terraform import elestio_fugu.my_fugu project_id,fugu_service_id
```
