...
description: |-
  Gogs is a lightweight self-hosted Git server that can be run on just about anything. The elestio_gogs resource allows the creation and management of Elestio Gogs services. The service uses the following docker image gogs/gogs https://hub.docker.com/r/gogs/gogs
---
...
```shell
# Import a Gogs service by specifying the Project ID it belongs to, and the Gogs service ID (spaced by a comma).
terraform import elestio_gogs.my_gogs project_id,gogs_service_id
```
