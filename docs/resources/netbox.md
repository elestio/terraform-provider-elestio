...
- `version` (String) This is the version of the software used as service. **Default** `v4.5-4.0.2`.
...
```shell
# Import a NetBox service by specifying the Project ID it belongs to, and the NetBox service ID (spaced by a comma).
terraform import elestio_netbox.my_netbox project_id,netbox_service_id
```
