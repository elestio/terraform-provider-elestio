...
page_title: "elestio_tracardi Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  TRACARDI is a new HOME for your customer data. TRACARDI is an API-first solution, a low-code / no-code platform aimed at any e-commerce business that wants to start using user data for marketing purposes.The elestio_tracardi resource allows the creation and management of Elestio Tracardi services. The service uses the following docker image tracardi/tracardi-api https://hub.docker.com/r/tracardi/tracardi-api
---
...
- `version` (String) This is the version of the software used as service. **Default** `0.8.0.5`.
...
```shell
# Import a Tracardi service by specifying the Project ID it belongs to, and the Tracardi service ID (spaced by a comma).
terraform import elestio_tracardi.my_tracardi project_id,tracardi_service_id
```
