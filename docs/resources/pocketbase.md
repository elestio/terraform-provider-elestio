...
description: |-
  Open Source backend for your next SaaS and Mobile app in 1 fileThe elestio_pocketbase resource allows the creation and management of Elestio PocketBase services. The service uses the following docker image augustodelg/pocketbase https://hub.docker.com/r/augustodelg/pocketbase
---
...
- `version` (String) This is the version of the software used as service. **Default** `0.20.0-rc3`.
...
```shell
# Import a PocketBase service by specifying the Project ID it belongs to, and the PocketBase service ID (spaced by a comma).
terraform import elestio_pocketbase.my_pocketbase project_id,pocketbase_service_id
```
