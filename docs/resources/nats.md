...
page_title: "elestio_nats Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  High-Performance server for NATS.io, the cloud and edge native messaging system.The elestio_nats resource allows the creation and management of Elestio Nats services. The service uses the following docker image nats https://hub.docker.com/r/nats
---

# elestio_nats (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/Nats-UFhx.png" width="100" /><br>High-Performance server for NATS.io, the cloud and edge native messaging system.<br><br>The **elestio_nats** resource allows the creation and management of Elestio Nats services. The service uses the following docker image [nats](https://hub.docker.com/r/nats)
...
- `version` (String) This is the version of the software used as service. **Default** `alpine`.
...
```shell
# Import a Nats service by specifying the Project ID it belongs to, and the Nats service ID (spaced by a comma).
terraform import elestio_nats.my_nats project_id,nats_service_id
```
