...
description: |-
  Penpot is the first open-source design and prototyping platform for cross-domain teams.The elestio_penpot resource allows the creation and management of Elestio Penpot services. The service uses the following docker image penpotapp/frontend https://hub.docker.com/r/penpotapp/frontend
---
...
- `version` (String) This is the version of the software used as service. **Default** `2.3.3`.
...
```shell
# Import a Penpot service by specifying the Project ID it belongs to, and the Penpot service ID (spaced by a comma).
terraform import elestio_penpot.my_penpot project_id,penpot_service_id
```
