...
description: |-
  PhotoPrism is an AI-powered app for browsing, organizing & sharing your photo collectionThe elestio_photoprism resource allows the creation and management of Elestio PhotoPrism services. The service uses the following docker image photoprism/photoprism https://hub.docker.com/r/photoprism/photoprism
---
...
```shell
# Import a PhotoPrism service by specifying the Project ID it belongs to, and the PhotoPrism service ID (spaced by a comma).
terraform import elestio_photoprism.my_photoprism project_id,photoprism_service_id
```
