...
page_title: "elestio_hoppscotch Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  Hoppscotch is a lightweight, web-based API development suite. The elestio_hoppscotch resource allows the creation and management of Elestio Hoppscotch services. The service uses the following docker image elestio/hoppscotch-app https://hub.docker.com/r/elestio/hoppscotch-app
---
...
```shell
# Import a Hoppscotch service by specifying the Project ID it belongs to, and the Hoppscotch service ID (spaced by a comma).
terraform import elestio_hoppscotch.my_hoppscotch project_id,hoppscotch_service_id
```
