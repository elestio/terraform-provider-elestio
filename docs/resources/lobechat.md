...
- `version` (String) This is the version of the software used as service. **Default** `0.1.16`.
...
```shell
# Import a LobeChat service by specifying the Project ID it belongs to, and the LobeChat service ID (spaced by a comma).
terraform import elestio_lobechat.my_lobechat project_id,lobechat_service_id
```
