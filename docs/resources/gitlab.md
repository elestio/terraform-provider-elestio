...
description: |-
  GitLab enables you to deliver software faster with better security and collaboration in a single platform.The elestio_gitlab resource allows the creation and management of Elestio Gitlab services. The service uses the following docker image gitlab/gitlab-ee https://hub.docker.com/r/gitlab/gitlab-ee
---
...
```shell
# Import a Gitlab service by specifying the Project ID it belongs to, and the Gitlab service ID (spaced by a comma).
terraform import elestio_gitlab.my_gitlab project_id,gitlab_service_id
```
