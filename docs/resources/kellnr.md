...
- `version` (String) This is the version of the software used as service. **Default** `5.0.0`.
...
```shell
# Import a Kellnr service by specifying the Project ID it belongs to, and the Kellnr service ID (spaced by a comma).
terraform import elestio_kellnr.my_kellnr project_id,kellnr_service_id
```
