...
description: |-
  VictoriaMetrics is a fast, cost-effective monitoring solution and time series databaseThe elestio_victoriametrics resource allows the creation and management of Elestio VictoriaMetrics services. The service uses the following docker image victoriametrics/victoria-metrics https://hub.docker.com/r/victoriametrics/victoria-metrics
---
...
```shell
# Import a VictoriaMetrics service by specifying the Project ID it belongs to, and the VictoriaMetrics service ID (spaced by a comma).
terraform import elestio_victoriametrics.my_victoriametrics project_id,victoriametrics_service_id
```
