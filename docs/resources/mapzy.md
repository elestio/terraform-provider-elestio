...
description: |-
  A simple, self-hostable store finder that cares about your privacy.The elestio_mapzy resource allows the creation and management of Elestio Mapzy services. The service uses the following docker image mapzy/mapzy https://hub.docker.com/r/mapzy/mapzy
---
...
```shell
# Import a Mapzy service by specifying the Project ID it belongs to, and the Mapzy service ID (spaced by a comma).
terraform import elestio_mapzy.my_mapzy project_id,mapzy_service_id
```
