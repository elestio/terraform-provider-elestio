...
description: |-
  A Fast and Scalable Web Platform by Extending NGINX with LuaJITThe elestio_openresty resource allows the creation and management of Elestio OpenResty services. The service uses the following docker image elestio/nginx-auto-ssl https://hub.docker.com/r/elestio/nginx-auto-ssl
---
...
```shell
# Import a OpenResty service by specifying the Project ID it belongs to, and the OpenResty service ID (spaced by a comma).
terraform import elestio_openresty.my_openresty project_id,openresty_service_id
```
