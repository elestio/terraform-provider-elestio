...
```shell
# Import a InvokeAI service by specifying the Project ID it belongs to, and the InvokeAI service ID (spaced by a comma).
terraform import elestio_invokeai.my_invokeai project_id,invokeai_service_id
```
