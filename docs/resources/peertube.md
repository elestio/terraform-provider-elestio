...
- `version` (String) This is the version of the software used as service. **Default** `production-bullseye`.
...
```shell
# Import a PeerTube service by specifying the Project ID it belongs to, and the PeerTube service ID (spaced by a comma).
terraform import elestio_peertube.my_peertube project_id,peertube_service_id
```
