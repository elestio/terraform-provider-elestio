...
page_title: "elestio_plane Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  An open-source software development tool to manage issues, sprints, and product roadmaps with peace of mindThe elestio_plane resource allows the creation and management of Elestio Plane services. The service uses the following docker image makeplane/plane-frontend https://hub.docker.com/r/makeplane/plane-frontend
---
...
- `version` (String) This is the version of the software used as service. **Default** `0.11`.
...
```shell
# Import a Plane service by specifying the Project ID it belongs to, and the Plane service ID (spaced by a comma).
terraform import elestio_plane.my_plane project_id,plane_service_id
```
