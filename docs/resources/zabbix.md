...
description: |-
  Zabbix is an enterprise-class open source distributed monitoring solution.The elestio_zabbix resource allows the creation and management of Elestio Zabbix services. The service uses the following docker image zabbix/zabbix-server-mysql https://hub.docker.com/r/zabbix/zabbix-server-mysql
---
...
- `version` (String) This is the version of the software used as service. **Default** `ubuntu-latest`.
