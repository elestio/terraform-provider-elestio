...
description: |-
  Matomo is a full-featured PHP MySQL software program that you download and install on your own webserverThe elestio_matomo resource allows the creation and management of Elestio Matomo services. The service uses the following docker image matomo https://hub.docker.com/r/matomo
---
...
```shell
# Import a Matomo service by specifying the Project ID it belongs to, and the Matomo service ID (spaced by a comma).
terraform import elestio_matomo.my_matomo project_id,matomo_service_id
```
