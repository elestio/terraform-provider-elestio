...
description: |-
  The Simple, Solid & Certified Kubernetes Distribution. Deploy and run Kubernetes workloads at any scale on any infrastructure. All batteries included. 100% open source & free.The elestio_k0s resource allows the creation and management of Elestio k0s services.
---
...
```shell
# Import a k0s service by specifying the Project ID it belongs to, and the k0s service ID (spaced by a comma).
terraform import elestio_k0s.my_k0s project_id,k0s_service_id
```
