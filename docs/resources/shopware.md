...
description: |-
  Shopware is a modern eCommerce platform connecting enterprise-level capabilities with flexibilityThe elestio_shopware resource allows the creation and management of Elestio Shopware services. The service uses the following docker image shyim/shopware https://hub.docker.com/r/shyim/shopware
---
...
- `version` (String) This is the version of the software used as service. **Default** `6.6.10.8-php8.2`.
...
```shell
# Import a Shopware service by specifying the Project ID it belongs to, and the Shopware service ID (spaced by a comma).
terraform import elestio_shopware.my_shopware project_id,shopware_service_id
```
