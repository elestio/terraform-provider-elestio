...
page_title: "elestio_tika Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  Apache Tika a content analysis toolkit detects and extracts metadata and text from over a thousand different file types (such as PPT, XLS, and PDF).The elestio_tika resource allows the creation and management of Elestio Tika services. The service uses the following docker image apache/tika https://hub.docker.com/r/apache/tika
---

# elestio_tika (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/Tika-hhxc.png" width="100" /><br>Apache Tika a content analysis toolkit detects and extracts metadata and text from over a thousand different file types (such as PPT, XLS, and PDF).<br><br>The **elestio_tika** resource allows the creation and management of Elestio Tika services. The service uses the following docker image [apache/tika](https://hub.docker.com/r/apache/tika)
...
- `version` (String) This is the version of the software used as service. **Default** `latest-full`.
...
```shell
# Import a Tika service by specifying the Project ID it belongs to, and the Tika service ID (spaced by a comma).
terraform import elestio_tika.my_tika project_id,tika_service_id
```
