...
page_title: "elestio_weaviate Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  Weaviate is an open-source vector database. It allows you to store data objects and vector embeddings from your favorite ML models, and scale seamlessly into billions of data objects.The elestio_weaviate resource allows the creation and management of Elestio Weaviate services. The service uses the following docker image semitechnologies/weaviate https://hub.docker.com/r/semitechnologies/weaviate
---
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `443/tcp`, `80/tcp`, `46834/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
```shell
# Import a Weaviate service by specifying the Project ID it belongs to, and the Weaviate service ID (spaced by a comma).
terraform import elestio_weaviate.my_weaviate project_id,weaviate_service_id
```
