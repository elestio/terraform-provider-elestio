...
description: |-
  Filestash is a Dropbox-like file manager that lets you manage your data anywhere it is located. The elestio_filestash resource allows the creation and management of Elestio Filestash services. The service uses the following docker image machines/filestash https://hub.docker.com/r/machines/filestash
---
...
- `version` (String) This is the version of the software used as service. **Default** `6b271d3`.
...
```shell
# Import a Filestash service by specifying the Project ID it belongs to, and the Filestash service ID (spaced by a comma).
terraform import elestio_filestash.my_filestash project_id,filestash_service_id
```
