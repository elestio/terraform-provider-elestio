...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `24306/tcp`, `24580/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `8.0`.
...
```shell
# Import a MySQL service by specifying the Project ID it belongs to, and the MySQL service ID (spaced by a comma).
terraform import elestio_mysql.my_mysql project_id,mysql_service_id
```
