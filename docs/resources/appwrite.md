...
description: |-
  Appwrite provides developers with a set of REST APIs to manage their core backend needs.The elestio_appwrite resource allows the creation and management of Elestio Appwrite services. The service uses the following docker image appwrite/appwrite https://hub.docker.com/r/appwrite/appwrite
---
...
```shell
# Import a Appwrite service by specifying the Project ID it belongs to, and the Appwrite service ID (spaced by a comma).
terraform import elestio_appwrite.my_appwrite project_id,appwrite_service_id
```
