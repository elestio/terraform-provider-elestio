...
description: |-
  Nebula is a scalable overlay networking tool with a focus on performance, simplicity and securityThe elestio_nebula resource allows the creation and management of Elestio Nebula services. The service uses the following docker image elestio/nebula-rest-api https://hub.docker.com/r/elestio/nebula-rest-api
---
...
```shell
# Import a Nebula service by specifying the Project ID it belongs to, and the Nebula service ID (spaced by a comma).
terraform import elestio_nebula.my_nebula project_id,nebula_service_id
```
