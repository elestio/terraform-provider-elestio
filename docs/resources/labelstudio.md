...
page_title: "elestio_labelstudio Resource - terraform-provider-elestio"
subcategory: "Services: AI/GPU"
description: |-
  Label Studio is a multi-type data labeling and annotation tool with standardized output formatThe elestio_labelstudio resource allows the creation and management of Elestio LabelStudio services. The service uses the following docker image heartexlabs/label-studio https://hub.docker.com/r/heartexlabs/label-studio
---
...
```shell
# Import a LabelStudio service by specifying the Project ID it belongs to, and the LabelStudio service ID (spaced by a comma).
terraform import elestio_labelstudio.my_labelstudio project_id,labelstudio_service_id
```
