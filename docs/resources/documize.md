...
description: |-
  Free Knowledge Management SoftwareThe elestio_documize resource allows the creation and management of Elestio Documize services. The service uses the following docker image debian https://hub.docker.com/r/debian
---
...
```shell
# Import a Documize service by specifying the Project ID it belongs to, and the Documize service ID (spaced by a comma).
terraform import elestio_documize.my_documize project_id,documize_service_id
```
