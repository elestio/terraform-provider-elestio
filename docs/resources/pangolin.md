...
page_title: "elestio_pangolin Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  Tunneled Mesh Reverse Proxy Server with Identity and Access Control and Dashboard UIThe elestio_pangolin resource allows the creation and management of Elestio Pangolin services. The service uses the following docker image fosrl/pangolin https://hub.docker.com/r/fosrl/pangolin
---

# elestio_pangolin (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/Pangolin-w9bd.png" width="100" /><br>Tunneled Mesh Reverse Proxy Server with Identity and Access Control and Dashboard UI<br><br>The **elestio_pangolin** resource allows the creation and management of Elestio Pangolin services. The service uses the following docker image [fosrl/pangolin](https://hub.docker.com/r/fosrl/pangolin)
...
- `firewall_enabled` (Boolean) Service firewall state. **Default** `false`.
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN), `80/tcp` (Let's Encrypt for custom domains). When `firewall_enabled` is `false`, set to `[]`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `1.4.0`.
...
```shell
# Import a Pangolin service by specifying the Project ID it belongs to, and the Pangolin service ID (spaced by a comma).
terraform import elestio_pangolin.my_pangolin project_id,pangolin_service_id
```
