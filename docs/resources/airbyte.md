...
subcategory: ""
description: |-
  Airbyte is an open-source data integration platform that syncs data from applications, APIs, and databases to warehouseselestio_airbyte is a preconfigured elestioservice resource (template_id: 157) running Airbyte ([`dockerimage: airbyte/server`](https://hub.docker.com/r/airbyte/server)).
---

# elestio_airbyte (Resource)

<img src="//cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/airbyte-new-logo-a0OK.svg" width="100" height="100" /><br/> Airbyte is an open-source data integration platform that syncs data from applications, APIs, and databases to warehouses<br/><br/>**elestio_airbyte** is a preconfigured elestio_service resource (`template_id: 157`) running **Airbyte** ([`docker_image: airbyte/server`](https://hub.docker.com/r/airbyte/server)).
...
```shell
# Import a Airbyte service by specifying the Project ID it belongs to, and the Airbyte service ID (spaced by a comma).
terraform import elestio_airbyte.my_airbyte project_id,airbyte_service_id
```
