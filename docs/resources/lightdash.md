...
description: |-
  Lighdash an open source alternative to Looker built using dbt.The elestio_lightdash resource allows the creation and management of Elestio Lightdash services. The service uses the following docker image lightdash/lightdash https://hub.docker.com/r/lightdash/lightdash
---
...
```shell
# Import a Lightdash service by specifying the Project ID it belongs to, and the Lightdash service ID (spaced by a comma).
terraform import elestio_lightdash.my_lightdash project_id,lightdash_service_id
```
