...
description: |-
  Ubuntu is a Linux distribution based on Debian and composed mostly of free and open-source software. The elestio_ubuntu resource allows the creation and management of Elestio Ubuntu services.
---
...
```shell
# Import a Ubuntu service by specifying the Project ID it belongs to, and the Ubuntu service ID (spaced by a comma).
terraform import elestio_ubuntu.my_ubuntu project_id,ubuntu_service_id
```
