...
- `version` (String) This is the version of the software used as service. **Default** `cpu-latest`.
...
```shell
# Import a Tei service by specifying the Project ID it belongs to, and the Tei service ID (spaced by a comma).
terraform import elestio_tei.my_tei project_id,tei_service_id
```
