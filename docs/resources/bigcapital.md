...
page_title: "elestio_bigcapital Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  Bigcapital is financial accounting with intelligent reporting for faster decision-making, an open-source alternative to Quickbooks, Xero, etc.The elestio_bigcapital resource allows the creation and management of Elestio Bigcapital services. The service uses the following docker image elestio/bigcapital-webapp https://hub.docker.com/r/elestio/bigcapital-webapp
---
...
```shell
# Import a Bigcapital service by specifying the Project ID it belongs to, and the Bigcapital service ID (spaced by a comma).
terraform import elestio_bigcapital.my_bigcapital project_id,bigcapital_service_id
```
