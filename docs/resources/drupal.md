...
description: |-
  Drupal is an open-source content management platform that supports a wide variety of websites.The elestio_drupal resource allows the creation and management of Elestio Drupal services. The service uses the following docker image bitnami/drupal https://hub.docker.com/r/bitnami/drupal
---
...
```shell
# Import a Drupal service by specifying the Project ID it belongs to, and the Drupal service ID (spaced by a comma).
terraform import elestio_drupal.my_drupal project_id,drupal_service_id
```
