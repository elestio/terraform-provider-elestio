...
page_title: "elestio_llana Resource - terraform-provider-elestio"
subcategory: "Services: Development"
description: |-
  API Wrapper for Databases - Llana is a no-code API wrapper that exposes a REST API for any database within minutes.The elestio_llana resource allows the creation and management of Elestio Llana services. The service uses the following docker image juicyllama/llana https://hub.docker.com/r/juicyllama/llana
---

# elestio_llana (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/llana-d95S.png" width="100" /><br>API Wrapper for Databases - Llana is a no-code API wrapper that exposes a REST API for any database within minutes.<br><br>The **elestio_llana** resource allows the creation and management of Elestio Llana services. The service uses the following docker image [juicyllama/llana](https://hub.docker.com/r/juicyllama/llana)
...
```shell
# Import a Llana service by specifying the Project ID it belongs to, and the Llana service ID (spaced by a comma).
terraform import elestio_llana.my_llana project_id,llana_service_id
```
