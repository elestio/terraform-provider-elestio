...
page_title: "elestio_garage Resource - terraform-provider-elestio"
subcategory: "Services: Applications"
description: |-
  (Mirror) S3-compatible object store for small self-hosted geo-distributed deployments.The elestio_garage resource allows the creation and management of Elestio Garage services. The service uses the following docker image dxflrs/garage https://hub.docker.com/r/dxflrs/garage
---

# elestio_garage (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/Garage-6RKg.png" width="100" /><br>(Mirror) S3-compatible object store for small self-hosted geo-distributed deployments.<br><br>The **elestio_garage** resource allows the creation and management of Elestio Garage services. The service uses the following docker image [dxflrs/garage](https://hub.docker.com/r/dxflrs/garage)
...
- `version` (String) This is the version of the software used as service. **Default** `v2.2.0`.
...
```shell
# Import a Garage service by specifying the Project ID it belongs to, and the Garage service ID (spaced by a comma).
terraform import elestio_garage.my_garage project_id,garage_service_id
```
