...
description: |-
  EspoCRM is a web application that allows users to see, enter and evaluate all your company relationships regardless of the type.The elestio_espocrm resource allows the creation and management of Elestio EspoCRM services. The service uses the following docker image espocrm/espocrm https://hub.docker.com/r/espocrm/espocrm
---
...
```shell
# Import a EspoCRM service by specifying the Project ID it belongs to, and the EspoCRM service ID (spaced by a comma).
terraform import elestio_espocrm.my_espocrm project_id,espocrm_service_id
```
