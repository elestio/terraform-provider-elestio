...
page_title: "elestio_milvus Resource - terraform-provider-elestio"
subcategory: "Services: Databases & Cache"
description: |-
  The High-Performance Vector Database Built for ScaleThe elestio_milvus resource allows the creation and management of Elestio Milvus services. The service uses the following docker image milvusdb/milvus https://hub.docker.com/r/milvusdb/milvus
---

# elestio_milvus (Resource)

<img src="https://cf.appdrag.com/dashboard-openvm-clo-b2d42c/uploads/milvus-xLTd.png" width="100" /><br>The High-Performance Vector Database Built for Scale<br><br>The **elestio_milvus** resource allows the creation and management of Elestio Milvus services. The service uses the following docker image [milvusdb/milvus](https://hub.docker.com/r/milvusdb/milvus)
...
- `firewall_user_rules` (Attributes Set) Firewall rules for the service. **Required ports (must use exactly `["0.0.0.0/0", "::/0"]`):** `22/tcp` (SSH), `4242/udp` (Nebula VPN). When `firewall_enabled` is `false`, set to `[]`. **Software default ports:** `80/tcp`, `443/tcp`, `19530/tcp`, `40318/tcp`. (see [below for nested schema](#nestedatt--firewall_user_rules))
...
- `version` (String) This is the version of the software used as service. **Default** `v2.4.7`.
...
```shell
# Import a Milvus service by specifying the Project ID it belongs to, and the Milvus service ID (spaced by a comma).
terraform import elestio_milvus.my_milvus project_id,milvus_service_id
```
