...
description: |-
  LimeSurvey is the most popular FOSS online survey tool on the web.The elestio_limesurvey resource allows the creation and management of Elestio LimeSurvey services. The service uses the following docker image owncloudops/limesurvey https://hub.docker.com/r/owncloudops/limesurvey
---
...
```shell
# Import a LimeSurvey service by specifying the Project ID it belongs to, and the LimeSurvey service ID (spaced by a comma).
terraform import elestio_limesurvey.my_limesurvey project_id,limesurvey_service_id
```
